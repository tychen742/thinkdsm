...
This repo vendors a tiny patch of `sphinx-thebe` to avoid duplicate injection of
the inline config script (which can happen in some Jupyter Book builds and leads
to `Identifier 'THEBE_JS_URL' has already been declared`).
"""
...
def init_thebe_core(app, env, docnames):
    ...
    """
    # In some builds, Sphinx can end up calling this hook more than once. Sphinx
    # de-duplicates JS files by filename, but not inline JS bodies, which leads to
    # duplicate `const` declarations and a hard SyntaxError.
    if getattr(app, "_dsm_sphinx_thebe_core_inited", False):
        return
    app._dsm_sphinx_thebe_core_inited = True
    ...
    thebe_config_lines = [
        f"var THEBE_JS_URL = \"{thebe_js_url}\"",
        f"var thebe_selector = \"{app.config.thebe_config['selector']}\"",
        f"var thebe_selector_input = \"{app.config.thebe_config['selector_input']}\"",
        f"var thebe_selector_output = \"{app.config.thebe_config['selector_output']}\"",
    ]
