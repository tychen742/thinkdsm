# Chapter 01 Organization

## Chapter Role

Chapter 01 is the course entry point. It gives students a business-oriented view of data science, introduces the course workflow, and connects programming concepts to later Python work.

## Learning Goals

Students should be able to:

1. Define data science as a combination of computing, statistics, domain expertise, and communication.
2. Describe how data science supports decisions in business and management contexts.
3. Identify the main phases of a data science workflow.
4. Explain why programming languages require precise syntax.
5. Distinguish interpreted and compiled execution at a high level.
6. Recognize expressions, statements, variables, number systems, and character encoding as foundations for later Python work.

## Sequence

1. `0100-intro.ipynb` - Introduction
   - Course and chapter orientation
   - Learning goals
   - Chapter flow
   - Glossary
   - Chapter overview slides
2. `0101-introds.ipynb` - Data Science
   - Why data science matters
   - What data science is
   - Data science workflows, roles, and tools
3. `0102-programming.ipynb` - On Programming
   - Natural and formal languages
   - Levels of abstraction
   - Interpreted and compiled execution
   - Programming constructs
   - Expressions and statements
   - Number systems
   - Character encoding
   - Further resources
4. `assignments/index.ipynb` - Assignments
   - Preview
   - Lab
   - Homework

## Exercise And Assignment Plan

- Preview quiz: checks Chapter 01 glossary terms before class.
- Lab: applies the data science lifecycle, programming concepts, binary notation, and character encoding. The submission UI grades the first five `thebe-interactive` cells directly.
- Homework: reinforces Chapter 01 with five true/false concept checks and five short coding questions covering workflow phases, expressions, number systems, and character encoding.

## Deferred Work

- Move `thinkpython.py` out of `chapters/01-intro/` into `materials/01/` and update notebook imports.
- Review whether `0101-introds.ipynb` should include section-level exercises following the current exercise rule.
- Confirm that Chapter 01 assignments should remain active even if Spring 2026 frozen-chapter constraints apply.
